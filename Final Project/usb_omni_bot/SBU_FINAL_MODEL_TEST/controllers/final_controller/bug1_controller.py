...
#for bug1
def update_min_distance(gps_values):
    global min_point
    global goal_position    
    
    new_distance = math.dist([goal_position[0], goal_position[1]], [gps_values[0], gps_values[1]])
    last_distance = math.dist([goal_position[0], goal_position[1]], [min_point[0], min_point[1]])
    
    if last_distance > new_distance : 
        #updating saved point
        min_point[0] = gps_values[0]
        min_point[1] = gps_values[1]

#for bug1
def hit_point_reached(gps_values):
    if abs(gps_values[1] - hit_point[1]) < 0.2 and abs(gps_values[0] - hit_point[0]) < 0.2:
        return 1
    return 0
...
if __name__ == "__main__":
    ...
    # DEFINE STATES HERE!
    FORWARD = 0
    WALL_FOLLOWING_LOOKING_MIN = 1
    WALL_FOLLOWING_GOING_MIN = 2
    ROTATE_TO_GOAL = 3
    ...
    while robot.step(TIME_STEP) != -1:
        ...
        if cur_state == FORWARD:
            ...
            if obs_infront(ir_value): 
                #hit a wall
                #save hitting point
                hit_point[0] = gps_values[0]
                hit_point[1] = gps_values[1]
                #save hitting point as min point
                min_point[0] = gps_values[0]
                min_point[1] = gps_values[1]
                got_far_from_hit = False
                cur_state = WALL_FOLLOWING_LOOKING_MIN
                init_wall_following()
        if cur_state == WALL_FOLLOWING_LOOKING_MIN:
            follow_wall(gps_values, compass_val, sonar_value, rflag)
            #checking distance of each point
            update_min_distance(gps_values)
            #if it has reached the last hitting point
            hit_reached = hit_point_reached(gps_values)
            if got_far_from_hit:
                if hit_reached == 1:
                    print("hit reached")
                    cur_state = WALL_FOLLOWING_GOING_MIN
            else:
                if hit_reached == 0:
                    got_far_from_hit = True
        if cur_state == WALL_FOLLOWING_GOING_MIN :
            follow_wall(gps_values, compass_val, sonar_value, rflag)
            #follow wall untill reaching min_point
            if abs(gps_values[1] - min_point[1]) < 0.2 and abs(gps_values[0] - min_point[0]) < 0.2:
                print("min reached")
                cur_state = ROTATE_TO_GOAL
